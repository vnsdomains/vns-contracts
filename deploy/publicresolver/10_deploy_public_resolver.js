...
module.exports = async ({getNamedAccounts, deployments, network}) => {
    ...
    const transactions = []
    transactions.push(await ens.setSubnodeOwner(ZERO_HASH, sha3('resolver.vlx'), deployer))
    transactions.push(await ens.setResolver(namehash.hash('resolver.vlx'), resolver.address))
    transactions.push(await resolver['setAddr(bytes32,address)'](namehash.hash('resolver.vlx'), resolver.address))
    transactions.push(await resolver.setInterface(namehash.hash('resolver.vlx'), "0x018fac06", ethregistrar.address))
